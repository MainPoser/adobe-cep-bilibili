    ...
    <el-container>
      ...
      <div class="material-image" style="flex: 1; text-align: center">
        <div v-for="item in materials" :key="item.id" class="block">
          ...
          <el-button :disabled="item.isDisabled" @click="downloadResource(item)"
                     class="demonstration">点击下载
          </el-button>
    ...
    handleSelectMenu(key) {
      ...
      this.$axios({
        ...
        for (let i = 0; i < this.materials.length; i++) {
          // 每个资源的下载进度条都是进度为0，不展示进度条，下载按钮可用
          this.materials[i].percentage = 0
          this.materials[i].show = false
          this.materials[i].isDisabled = false
        }
      }).catch(err => {
        console.log(err)
      })
